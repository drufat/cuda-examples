...
void gears_initialize(Gears *g)
{
    ...
    g->gear1 = gears_make(reflectance1, 1.0, 4.0, 1.0, 0.7, 20);
    g->gear2 = gears_make(reflectance2, 0.5, 2.0, 2.0, 0.7, 10);
    g->gear3 = gears_make(reflectance3, 1.3, 2.0, 0.5, 0.7, 10);
...
void gears_paint(const Gears *g)
{
    ...
    glPushMatrix();
    glRotated(g->xRot / 16.0, 1.0, 0.0, 0.0);
    glRotated(g->yRot / 16.0, 0.0, 1.0, 0.0);
    glRotated(g->zRot / 16.0, 0.0, 0.0, 1.0);

    gears_draw(g->gear1, -3.0, -2.0, 0.0, g->gear1Rot / 16.0);
    gears_draw(g->gear2, +3.1, -2.0, 0.0, -2.0 * (g->gear1Rot / 16.0) - 9.0);

    glRotated(+90.0, 1.0, 0.0, 0.0);
    gears_draw(g->gear3, -3.1, -1.8, -2.2, +2.0 * (g->gear1Rot / 16.0) - 2.0);
