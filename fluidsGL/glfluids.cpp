...
void initParticles(float2 *p, int dx, int dy) {
  ...
  auto myrand = []() -> float { return qrand() / (float)RAND_MAX; };
...
GLFluids::GLFluids(QWidget *parent) : QGLWidget(parent), QGLFunctions() {
  vbo = 0;
  ...
  cudaMemcpy(dvfield, hvfield, sizeof(float2) * DS, cudaMemcpyHostToDevice);
...
void GLFluids::reset() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float2) * DS, particles,
               GL_DYNAMIC_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);
